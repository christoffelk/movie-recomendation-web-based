...
class FCM(BaseModel):
    ...
    def fit(self, X: NDArray) -> None:
        ...
        self.u = np.divide(self.u,np.tile(self.u.sum(axis=1)[np.newaxis].T,self.n_clusters))
        
        print(np.tile(self.u.sum(axis=1)[np.newaxis].T,self.n_clusters))
...
data = pd.read_csv("ratings.csv", delimiter=',', usecols= [2])
X = data.values
